...
class InputImagesSimple extends InputWidget
{
	public $buttonName;
	public $buttonOptions = [];
	public $multiple = false;
	public $uploadHandler = 'uploader/upload-image';
	...
	public function run()
	{
		...
		if ($this->multiple) {
			$this->getView()->registerJs("andrewdanilov.inputImages.register(" . Json::encode($this->options['id']) . ", InputImagesSimpleHandler);");
		} else {
			$this->getView()->registerJs("andrewdanilov.inputImages.register(" . Json::encode($this->options['id']) . ", InputImageSimpleHandler);");
		}

        if (empty($this->widgetBody)) {
            // Add empty input to widget wrapper if there is no images at all
            return $this->buildWidget($this->getInput());
        } else {
    ...
    private function getInput($image='')
    {
        if ($this->hasModel()) {
	 ...
	 * @return string
	 */
	private function buildWidget($extra_content='')
	{
